...
public class FastestGradient {
    ...
    public static Double run(double eps,
                             ...
                             double L) throws InvocationTargetException, IllegalAccessException {
        vectors.clear();
        ...
        while (gradient.module(x) >= eps) {
            ...
            vectors.add(x);
            alpha = OneDimensionalOptimization.methodGoldenRatio(a -> function.apply(gradient.evaluate(X).multiplyConst(-1 * a).add(X)), 0, L / 2, eps);
    ...
    public static void run(Function<VectorNumbers, Double> function, Gradient gradient, double L) throws InvocationTargetException, IllegalAccessException {
        vectors.clear();
